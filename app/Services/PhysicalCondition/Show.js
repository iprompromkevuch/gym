...
class Show {
    async execute(request, auth, response) {
        try {
            ...
            const result = physConditions.rows[0] ? physConditions.rows[0] : null;
            
            return response.status(200).json({ 
                data : result ? dump.dumpPhysCondition(result) : {}
            });
        } catch (err) {
  ...
  *     security:
  *       - bearerAuth: []
  *     summary: Get info of LATEST physical condition
  *     parameters:
  *       - name: id
  *         description: id of user (search phys conditions by USER id). If you have permissions, you will get LATEST user physical conditions
  *         in: path
  *         required: true
  *     responses:
  *       200:
